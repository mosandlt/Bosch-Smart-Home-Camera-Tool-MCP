...
The [publish-pypi workflow](../.github/workflows/publish-pypi.yml) fires automatically on any `v*.*.*` tag push. Builds the wheel + sdist, then publishes to PyPI via **OIDC Trusted Publisher** — no API token in repository secrets.
...
Also create a GitHub environment named `pypi` in the repo settings (Settings → Environments → New environment → name it `pypi`). The environment is optional for PyPI's side but matches the `environment: pypi` field in the workflow.
