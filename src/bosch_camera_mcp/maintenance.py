...
announcement so an LLM caller can show a specific reason ("Wartung 07:00-10:00
MESZ") instead of a generic explanation when the cloud returns 5xx errors.
...
# Boards used for service status. Order = preference; first success wins.
...
class MaintenanceWindow:
    """Parsed maintenance/incident announcement from a community feed."""
    ...
    *,
    ...
    try:
        ...
        if resp.status_code != 200:
            _LOGGER.debug("Maintenance HTTP %s for %s", resp.status_code, url)
            return None
        return resp.content
...
async def async_fetch_maintenance(
    *,
    ...
    caller should keep any last cached value rather than flipping state on a
    transient miss of the community site.

    Uses httpx.AsyncClient (already a transitive dependency of the mcp package).
    No external session required; manages its own connection lifecycle.
    """
    best: MaintenanceWindow | None = None
    async with httpx.AsyncClient(follow_redirects=True) as client:
        for url in RSS_FEEDS:
            body = await _fetch_one_async(client, url, timeout_s=timeout_s)
            if body is None:
                continue
            parsed = _parse_feed_body(body, url)
        ...
        for url in HTML_FALLBACKS:
            ...
            if body is None:
                continue
            parsed = _parse_html_fallback(body, url)
            if parsed and (best is None or _prefers(parsed, best)):
                best = parsed
    return best
